  ...
  return (
    <nav className="navbar flex items-center justify-between py-6 px-12 border-b font-sans">
      ...
      <div className="flex gap-7 text-lg items-center">
        <Link href="/products" className="hover:underline text-brown-800">Products</Link>
        <Link href="/events" className="hover:underline text-brown-800">Events</Link>
        <Link href="/cart" className="flex items-center gap-1 text-brown-800">
          Cart
          <span className="bg-green-700 ml-1 text-white rounded-full px-2 text-xs">{items.length}</span>
        </Link>
        {user ? (
          <>
